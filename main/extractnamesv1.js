...
async function queryAutocomplete(query) {
  try {
    totalRequests++;
    const url = `${BASE_URL}/${VERSION}/autocomplete?query=${encodeURIComponent(
      query
    )}`;
    const response = await axios.get(url);
    successfulRequests++;
    console.log(
      `Request #${totalRequests} successful for prefix "${query || "empty"}"`
    );
...
async function extractAllNames() {
  ...
  while (queue.length > 0) {
    ...
    for (const { prefix, results } of batchResults) {
      ...
      if (completedPrefixes % 10 === 0) {
        console.log(
          `Progress: ${completedPrefixes} prefixes processed, ${
            queue.length
          } in queue, ${trie.getAllWords().length} names found`
        );
      }
      if (results.length === MAX_RESULTS && prefix.length < MAX_PREFIX_LENGTH) {
        for (let c = "a".charCodeAt(0); c <= "z".charCodeAt(0); c++) {
          const nextPrefix = prefix + String.fromCharCode(c);
  ...
  fs.writeFileSync("v1_names_trie.json", JSON.stringify(allNames, null, 2));
  ...
  console.log(
    `Time taken: ${Math.floor(duration / 60)} minutes ${Math.floor(
      duration % 60
    )} seconds`
  );
  console.log("------------------------\n");
  console.log(`Number of results found in v1 autocomplete: ${allNames.length}`);
